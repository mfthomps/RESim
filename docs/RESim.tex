...
Three network switches are created.  By default, computer ethernet interface is connected to a switch, in the order
of definition.  Simics CLI variables are assigned to each computer ethernet link using the convention {\tt TARGET\_eth0} where {\tt TARGET}
is the value of the configuration file section header for that component.  TBD: add {\tt NETWORK} section to the configuration file?
...
triggered by system events.  For example, a script that loads selected kernel modules could be augmented to
...
regular expression via the {\tt write} or {\tt read} system calls.  Note in all the subfunctions listed
below, the trigger string identifies the read or write operation that triggers the action. 
...
in a read or write buffer (per the <operation> field), and a separate substring is also found.  If the trigger string is
...
RESim pauses then after all computers in the simulation have reached this initial state.  Note though that
...
at which a dynamic modification was required.
