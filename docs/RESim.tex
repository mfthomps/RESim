...
IP addresses set using {\tt ip} or {\tt ifconfig} commands, which can be seen using the {\tt showNets} command.  Trace files are created in the /tmp directory.  During a trace, if you use the Simics stop and
...
see network ports being listened to. Use the {\tt showProcTrace} command to view a report on all processes and their IO usage.  Either exit RESim or use {\tt flushTrace} to make sure the trace log is complete. 
...
The resulting reports may aid you in finding processes of interest, and identifying the programs that they execute.  These can be opened in tools such as IDA Pro or Ghidra to help you understand
their operation.
...
\item {\tt precall([pid]} - Reverse until prior to system call for given or current PID.  Useful when process is waiting for input within the kernel
and you want to track IO.  RESim needs to catch the call to the IO operation, and cannot do that if the call has already been made.  Note however that
this, like all reversing calls, assumes you have recorded time into which you can reverse.
...
The trackIO function will break simulation after 50K cycles with no data references.
...
the size of the file.  The {\tt trackIO} function is then invoked for the given FD.  Intended use is to rapidly observe execution paths for variations in
...
command will fail to detect the receive -- though references to already defined data buffers will be added to the Watch Marks.  To ensure that you record
the read, skip to the last Watch Mark (e.g., use {\tt goToDataMark(-1)} prior to issuing the next {\tt trackIO} command.

Much of the above presupposes you begin your data tracking before the receiving thread invokes the kernel to read data.  That is often not the case, e.g., the thread
is in the kernel on a blocking recv waiting for data.  RESim will not catch those receives in a {\tt trackIO} or {\tt runToIO} command.
The easiest solution (at the moment) is to send a stub packet after issuing the {\tt runToIO} command.  Then send the stub a second time, and it will be processed
by the {\tt runToIO} fuction, leaving the thread in the state of recently returning from the kernel read of the second stub.
...
\subsection{Tracking IO while in a blocking read}
You want to track IO on FD 4.  However, if the process is currently blocked on a read (or recv or whatever) of FD 4, the syscall will not
be seen.  The call will return without being caught by a syscall exit hap.  We could start going backwards until prior to the syscall -- but
how do we even know we are in a read syscall, and not a select or such?  One crude strategy may be to reverse until one of our threads is
exiting the kernel?  But that assumes we have runway behind.

For now, it is up to the user to work around this. For example, use {\tt runToIO(4)}, send a packet, and then use {\tt trackIO(4)} and send
the desired packet.
