...
if [[ -f $ida_db_path ]] || [[ -f $other_ida_db_path ]];then
    ...
    echo $idacmd -L/tmp/idaGadets.log -A -a -S$RESIM_DIR/simics/ida/idaGadgets.py $ida_db_path
    "$idacmd" -L/tmp/idaGadgets.log -A -S$RESIM_DIR/simics/ida/idaGadgets.py "$ida_db_path" || tail /tmp/idaGadgets.log && exit 1
else
    echo "No IDA db at $ida_db_path  create it."
    mkdir -p "$RESIM_IDA_DATA/$root_parent/$root_dir/$target_base"
    echo $idacmd -L/tmp/idaDump.log -A -o"$ida_db_path" -S$RESIM_DIR/simics/ida/idaGadets.py "$target"
    "$idacmd" -L/tmp/idaGadets.log -A -o"$ida_db_path" -S$RESIM_DIR/simics/ida/idaGadets.py "$target" || tail /tmp/idaGadets.log && exit 1
fi
