...
class ScanMark():
    def __init__(self, src, dest, count, buf_start, value, sp):
        self.src = src    
        self.dest = dest    
        self.count = count    
        self.buf_start = buf_start    
        ...
        if value is not None:
            if type(value) is str:
                ...
                value_str = 'value: 0x%x' % value
        if dest is None:
            self.msg = 'sscanf failed to parse from 0x%08x' % src
        else:
            self.msg = 'sscanf src 0x%x to 0x%08x %d bytes %s' % (src, dest, count, value_str)
...
class WatchMarks():
    ...
    def saveMarks(self, fpath):
        ...
        with open(fpath, 'w') as fh:
            ...
            for mark in self.stale_marks:
                ...
                if mark.cycle < prev_cycle:
                    fh.write('ERROR ***** next cycle 0x%x less than previous 0x%x' % (mark.cycle, prev_cycle))
    ...
    def showMarks(self, old=False, verbose=False):
        ...
        if old:
            for mark in self.stale_marks:
                ...
                the_str = mark.mark.getMsg().encode('utf-8', 'ignore')
                if mark.cycle < prev_mark_cycle:
                    print('ERROR ****** next mark cycle 0x%x less than previous 0x%x' % (mark.cycle, prev_mark_cycle))
                print('%d%s%s  ip:0x%x tid:%s' % (i, cycle, the_str, mark.ip, mark.tid))
                i += 1
                prev_mark_cycle = mark.cycle
    ...
    def sscanf(self, src, dest, count, buf_start, value=None):
        #sp, base = self.getStackBase(dest)
        if dest is not None:
            sp = self.isStackBuf(dest)
        else:
            sp = None
        sm = ScanMark(src, dest, count, buf_start, value, sp)        
