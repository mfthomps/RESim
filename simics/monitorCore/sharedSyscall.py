...
class SharedSyscall():
    ...
    def doSockets(self, exit_info, eax, pid):
        ...
        if exit_info.callnum == self.task_utils.syscallNumber('socketcall'):
            ...
            socket_syscall = self.top.getSyscall(socket_callname)
        ...
        if socket_callname == "socket" and eax >= 0:
            ...
            optval_val = self.mem_utils.readWord32(self.cpu, exit_info.retval_addr)
            trace_msg = ('\treturn from socketcall SETSOCKOPT pid:%d optval_val 0x%x  eax: 0x%x\n' % (pid, optval_val, eax))
        else:
    ...
    def exitHap(self, dumb, third, forth, memory):
        ...
        if exit_info is None:
            ...
            if self.isPendingExecve(pid):
                self.lgr.debug('exitHap call reschedule from execve?  for pid %d  Remove pending' % pid)
        ...
        #self.lgr.debug('exitHap callnum %d name %s  pid %d' % (exit_info.callnum, callname, pid))
        if callname == 'clone':
            ...
            if eax >= 0 and exit_info.retval_addr is not None:
                ...
                if exit_info.call_params is not None and exit_info.call_params.break_simulation and self.dataWatch is not None:
                    ...
                    if eax < 1024:
                        self.lgr.debug('syscall read check diddler count %d' % eax)
