...
been initialized.  This one calls playAFL for searching
'''
import os
def onedone(top):
    ...
    search_list=os.getenv('ONE_DONE_PARAM8')
    if not os.path.isfile(search_list):
        print('no file at %s' % search_list)
        top.quit()
    here = os.getcwd()
    ...
    with open('logs/onedonePlay.log', 'w') as fh:
        ...
        if protocol == 'tcp': 
            ...
            top.playAFL(base, parallel=True, only_thread=only_thread, fname=program, target=target, targetFD=targetFD, count=count, no_page_faults=no_page_faults, search_list=search_list)
